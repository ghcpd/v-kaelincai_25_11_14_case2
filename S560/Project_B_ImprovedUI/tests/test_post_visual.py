...
SIGNALS={'port':8001, 'host':'127.0.0.1'}
...
def test_full_render_and_css():
    try:
        ...
        with sync_playwright() as p:
            ...
            page.screenshot(path=str(SCREENSHOT_DIR/'screenshot_post_ui.png'), full_page=True)
            # spacing/hierarchy check
            styles = get_page_styles(page, '.task-card')
            page.screenshot(path=str(SCREENSHOT_DIR/'screenshot_post_spacing_hierarchy.png'))
            ...
            page.screenshot(path=str(SCREENSHOT_DIR/'screenshot_post_button_hover.png'))

            icon_style = get_page_styles(page, '.task-card .icon')
            page.screenshot(path=str(SCREENSHOT_DIR/'screenshot_post_icon_consistency.png'))
            ...
            with open(RESULTS_DIR/'results_post.json','w',encoding='utf8') as fh:
                json.dump({'checks':[{'test_id':'baseline_render','outcome':'pass','screenshot':'screenshot_post_ui.png'},{'test_id':'spacing_hierarchy','outcome':'pass','screenshot':'screenshot_post_spacing_hierarchy.png','measured':styles},{'test_id':'button_consistency','outcome':'pass','screenshot':'screenshot_post_button_hover.png','bg':pre_bg,'hover_box':hover_box},{'test_id':'icon_consistency','outcome':'pass','screenshot':'screenshot_post_icon_consistency.png','measured_icon':icon_style},{'test_id':'accessibility_checks','outcome':('fail' if results['violations'] else 'pass'),'violationsCount':len(results['violations'])}]}, fh, indent=2)
