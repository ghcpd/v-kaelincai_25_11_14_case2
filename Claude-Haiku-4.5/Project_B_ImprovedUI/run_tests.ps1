# PowerShell execution script for Project B - Improved UI
Write-Host "=== Running Project B - Improved UI Tests ===" -ForegroundColor Cyan
...
Write-Host "Starting improved UI tests at $(Get-Date)" | Tee-Object $logFile

try {
    # Run visual tests
    Write-Host "Running visual tests..." -ForegroundColor Green | Tee-Object $logFile -Append
    Set-Location tests
    python test_post_visual.py | Tee-Object ..\$logFile -Append

    # Run CSS assertion tests
    Write-Host "Running CSS assertion tests..." -ForegroundColor Green | Tee-Object ..\$logFile -Append
    python test_post_css_assertions.py | Tee-Object ..\$logFile -Append

    # Run accessibility tests
    Write-Host "Running accessibility tests..." -ForegroundColor Green | Tee-Object ..\$logFile -Append
    python test_post_a11y.py | Tee-Object ..\$logFile -Append

    Set-Location ..
    
    Write-Host "Improved UI tests completed at $(Get-Date)" -ForegroundColor Cyan | Tee-Object $logFile -Append
    Write-Host "Results saved to:" -ForegroundColor Green
    Write-Host "  - screenshots/" -ForegroundColor White
    Write-Host "  - logs/log_post.txt" -ForegroundColor White
    Write-Host "  - results/results_post.json" -ForegroundColor White
    Write-Host "  - results/css_assertions_post.json" -ForegroundColor White
    Write-Host "  - results/a11y_results_post.json" -ForegroundColor White
